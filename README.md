...
### Set Slack Bot Token
- Create an App in api.slack.com/apps or use your existing app if you have one
- Go to `OAuth & Permissions`
- Ensure it has these permissions
  - `chat:write`
  - `chat:write:public`
- Obtain the bot token from `OAuth & Permissions section`
- Install this app into your slack workspace 

### To Deploy on the cluster
**Build and push your image to the location specified by `IMG`:**

```sh
make docker-build docker-push IMG=<some-registry>/slack-notifier:tag
```
...
```sh
make deploy IMG=<some-registry>/slack-notifier:tag
```
...
```sh
make build-installer IMG=<some-registry>/slack-notifier:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/slack-notifier/<tag or branch>/dist/install.yaml
```
